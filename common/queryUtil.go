...
func QueryPageUtil(model *interface{}, currentPage, pageSize int) map[string]interface{} {
	models := []interface{}{}
	o := orm.NewOrm()
	qs := o.QueryTable(model).Filter("is_delete", 0)
	qs.Limit(pageSize).Offset((currentPage - 1) * pageSize).All(&models)
	count, _ := qs.Count()
	countPage := int(math.Ceil(float64(count) / float64(pageSize)))
	pageMap := Paginator1(currentPage, pageSize, count)
	prePage := 1
	if currentPage == 1 {
		prePage = currentPage
	...
	pageMap["prePage"] = prePage
	pageMap["nextPage"] = nextPage
	return pageMap
}

//分页算法，当前页、每页数、总数
func Paginator1(currentPage, pageSize int, count int64) map[string]interface{} {
